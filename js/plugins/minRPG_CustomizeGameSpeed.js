 ...
 * @desc 初期値=2。ツクールデフォルトに対する、敵キャラの状態異常や倒れエフェクト速度です。
 * @default 2
 * 
 * @param 標準戦闘武器モーションスピード
 * @desc 初期値=1。ツクールデフォルトに対する、サイドビューキャラの武器・戦闘モーション速度です。
 * @default 1
 * 
 * @param 標準戦闘移動スピード
 * @desc 初期値=2。ツクールデフォルトに対する、サイドビューキャラの移動速度です。
 * @default 2
 * 
 * @param 標準戦闘アニメーションスピード
 * @desc 初期値=1。ツクールデフォルトに対する、スキルエフェクトのアニメーション倍率です。
 * @default 1
 * 
 * 
 * @param ↓早送りスピード倍率
 ...
 * 
 * @param 早送りイベントスピード
 * @desc 初期値=2。ツクールデフォルトに対する、早送り時のイベント速度倍率を調整します。
 * @default 2
 * 
 * @param 早送り戦闘ログスピード
 ...
 * @desc 初期値=6。ツクールデフォルトに対する、早送り時のサイドビューキャラの武器モーション速度です。
 * @default 6
 * 
 * @param 早送り戦闘移動スピード
 * @desc 初期値=8。ツクールデフォルトに対する、早送り時のサイドビューキャラの移動速度です。
 * @default 8
 * 
 * @param 早送り戦闘アニメーションスピード
 * @desc 初期値=2。ツクールデフォルトに対する、早送り時のスキルエフェクトのアニメーション倍率です。
 * @default 2
 * 
 * 
 * @param ↓早送りボタンの有効/無効化
    ...
    */
    ...
    var getMessageWaitEscapeCharacterSpeed = function () {
        ...
        if (_VarNo_GameSpeed != null && _VarNo_GameSpeed >= 1 && _VarNo_GameSpeed < $dataSystem.variables.length) {
            ...
            _addCount = Number.parseInt(_gameSpeed / 5); // ★戦闘スピード1-9が設定してあれば、それに5を割ったもの。
            if (_gameSpeed === Infinity || _gameSpeed === -Infinity) { _addCount = 0; } // 無限大なら、0で。
            else if (_gameSpeed === 0) { _addCount = 0; } // 0が入っていたら、0で。
            else if (_gameSpeed === 5) { _addCount = 0; } // 5が入っていたら、0で。
            else if (_gameSpeed === 10) { _addCount = 999; } // 最速の10が入っていたら、999で。
            else if (_gameSpeed < 0) { _addCount = 0; } // 1未満なら0に。
            else if (_gameSpeed > 99) { _addCount = 999; } // 1000以上になっても意味ないと思う。
            // _speedを足す。
            _speed += _addCount;
        }
        ...
        if (isFastRapidButton()) {
            ...
            if (_VarNo_Rapid_BattleRate != null && _VarNo_Rapid_BattleRate >= 1 && _VarNo_Rapid_BattleRate < $dataSystem.variables.length) {
                _addCount = Number.parseInt($gameVariables.value(_VarNo_Rapid_BattleRate));
                // 変数の値に変な値が入っていないか、随時チェック。
                if (_addCount === Infinity || _addCount === -Infinity) { _addCount = 0; } // 無限大なら、0。
                else if (_addCount === 0) { _addCount = 0; } // 0が入っていたら、0で。
                else if (_addCount < 0) { _addCount = 1; }   // 1未満なら0に。
                else if (_addCount > 99) { _addCount = 999; } // 1000以上になっても意味ないと思う。0
            }else{
                _addCount = 0;
            }
            // _speedを足す。
        ...
        */
        ...
        Window_BattleLog.prototype.updateWaitCount = function () {   // 元メソッドの返り値は、this._waitCount>0ならtrue、それ以外ならfalseだよ。
            if (this._waitCount > 0) {
                ...
                // merusaiaの改変後は下。
                // 1. _waitCountを減らすデフォルトは、2。
                var _reduceCount = _BATTLE_LOGSPEED_DEFAULT;
                // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                if (_VarNo_GameSpeed != null && _VarNo_GameSpeed >= 1 && _VarNo_GameSpeed < $dataSystem.variables.length) {
                    var _gameSpeed = $gameVariables.value(_VarNo_GameSpeed);
                    // 変数の値に変な値が入っていないか、随時チェック。.valueで || 0 としているため、nullやundefinedやNaNは0になる。判定は±Infinityだけで十分
                    _reduceCount = Number.parseInt(_gameSpeed / 3); // ★戦闘スピード1-9が設定してあれば、それに3を割ったもの。
                    ...
                    else if (_gameSpeed === 10) { _reduceCount = _BATTLE_LOGSPEED_DEFAULT; } // 最速の10が入っていたら、99で。
                ...
                if(_reduceCount <= 0){
                    if(_isDebugMode()){
                        console.log('■_reduceCount：' + (_reduceCount) + ' が0以下です。0だと止まってしまうので、 1にします。');
                    }
        ...
        Sprite_Animation.prototype.update = function () {           // 元メソッドにreturnはないよ。
            ...
            // merusaiaの追加行は下。
            // 1. 戦闘スピードのデフォルトは1。
            var _speed = _BATTLE_ANIMATIONSPEED_DEFAULT+2;
            // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
            if (_VarNo_GameSpeed != null && _VarNo_GameSpeed >= 1 && _VarNo_GameSpeed < $dataSystem.variables.length) {
                var _gameSpeed = $gameVariables.value(_VarNo_GameSpeed);
        ...
        Sprite_Actor.prototype.updateMove = function () {            // 元メソッドにreturnはないよ。
            ...
            if (!bitmap || bitmap.isReady()) {
                ...
                if(_speed > _FRAME_UPDATENUM_MAX * 100){
                    _speed = _FRAME_UPDATENUM_MAX * 100;
                }
                // 5. _speed回だけ、更新処理を呼び出す。（体感ゲームスピードが_speed倍になる）
                for (var i = 2; i < _speed; i++) {
                    Sprite_Battler.prototype.updateMove.apply(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
