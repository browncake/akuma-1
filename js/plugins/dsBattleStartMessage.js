...
/*:
 * @plugindesc 戦闘開始時のメッセージ表示を変更するプラグイン ver1.00
 * @author 道楽
 *
 * @param Max Lines
 * @desc 最大表示行数
...
(function(ns) {
	...
	ns.Window_EnemyName = (function() {
		...
		{
			return this.fittingHeight(ns.Param.MaxLines);
		};
		...
		{
			return gMessagePosY + numLines * (this.lineHeight() + ns.Param.LineSpace);
		};
		...
		{
			this.refresh();
			this._showCount = ns.Param.ShowCount;
			this._showWait = ns.Param.ShowWait;
		};
		...
		Window_EnemyName.prototype.refresh = function()
		{
			...
			if ( this._showNames.length > 0 )
			{
				...
				this._showNames.forEach(function(text, idx) {
					var y2 = gMessagePosY + (lineHeight + ns.Param.LineSpace) * idx;
					this.drawBackground(0, y2, this.textWidth(text), lineHeight);
					this.drawText(text, 8, y2);
				}, this);
			}
		};
	...
	{
		this._enemyNameWindow = new ns.Window_EnemyName();
